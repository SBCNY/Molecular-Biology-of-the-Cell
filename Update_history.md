...
<br>
May 25th, 2023
- Dataset-specific colors can be uploaded using the functionalities of the menu 'Read data'. It is now possible to define colors in hexadecimal numbers that will be mapped to the closest C# colors by the application. This allows visualization of MBCO enrichment results in the same colors used within other lanuguages, e.g. R or Python. Simply convert the color names to hexadecimal numbers (e.g. using col2hex ('gplots' library) in R) and add these to a column within the dataset file that contains the colors.
