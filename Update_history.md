...
June 3, 2025
-	We added the new menu ‘Ontologies / Species’ that allows selection of 11 species for the 8 selectable ontologies (or ontology namespaces). If gene-pathway annotations do not exist for a species of interest, the application will map human gene-pathway annotations to the species of interest using NCBI and, if available for the selected species, MGI orthologues.
...
-	For further automation, the application can now be completely executed from the command line, using supplied parameter settings, mimicking activation of the ‘Clear, read & analyze’-button.
...
October 15, 2024
- SCP networks generated by our application can be visualized using the graph editor yED. Within yED you can zoom out five times from the 1:1 level before pie charts are visualized as gray squares and node labels disappear. This might limit the number of nodes that can simultaneously be investigated on smaller screens. To offer more flexibility, we now added a text box to the 'SCP networks' menu that allows specification of the font sizes for node labels, in addition to the text box for the specification of node diameters that we added in July.
