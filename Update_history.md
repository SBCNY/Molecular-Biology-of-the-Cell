...
<br>
March 20th, 2023
- Heatmaps allow comparison of -log10(p-values) or ranks between the different datasets. If a heatmap contains too many rows and/or columns, it is split in multiple heatmap blocks. The blocks are now first sorted by columns, then by rows, allowing easier comparison of the results for the same SCP accross all datasets.
