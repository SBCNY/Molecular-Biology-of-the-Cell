...
- Within the ‘Enrichment’ menu the user can now define minimum and maximum sizes for Gene Ontology (GO) terms (i.e., biological processes, cellular components and molecular functions). Only terms meeting those cutoffs will be analyzed for enrichment of user-supplied gene lists and used for integration of predicted terms into GO hierarchical networks. Selected cutoffs are applied after population of parent terms with genes of their child terms across all three namespaces. For consistent enrichment results under different size cutoffs, we now re-add all genes of the selected namespace as background genes, if they were removed, because they only map to terms outside of the selected cutoffs.
