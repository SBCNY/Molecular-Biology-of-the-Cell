...
- The application now additionally accepts Reactome and two user-supplied custom ontologies that can be selected using the ‘Enrichment’ menu.
- Within the ‘Enrichment’ menu the user can now define minimum and maximum sizes for Gene Ontology (GO) terms (i.e., biological processes, cellular components and molecular functions). Only terms meeting those cutoffs will be analyzed for enrichment of user-supplied gene lists and used for integration of predicted terms into GO hierarchical networks. Selected cutoffs are applied after population of parent terms with genes of their child terms across all three namespaces. For consistent enrichment results under different size cutoffs, we now re-add all genes of the selected namespace as background genes, if they were removed, because they only map to terms missing the selected cutoffs.
- GO term networks will be generated from the GO directed acyclic graph using parent-child relationships. The ‘SCP networks’ menu now also allows to select to additionally include regulatory relationships. The population of parent terms with the genes of their child terms is not affected by this selection.
- Since networks of pathway relationships of other ontologies can be very large, the user can now specify within the ‘SCP networks’ menu to connect predicted pathways only by pathway nodes lying between them, instead of adding all ancestor pathways that fulfill selected size requirements.
...
-	A new button allows to select if the text sizes of node labels should increase with increasing node areas.
-	The user can now specify the maximum diameter of an SCP node and if SCP node areas should be scaled uniquely or consistently across different integration groups (i.e., networks) for the same enrichment algorithm (i.e., dynamic or standard).
