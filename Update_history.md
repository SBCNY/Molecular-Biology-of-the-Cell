...
<br>
October 15, 204
- SCP networks generated by our application can be visualized using the graph editor yED. Within yED you can zoom out 5 times from the 1:1 level before pie charts are visualized as gray squares and node labels disappear. This might reduce the number of nodes that can be investigated on smaller screens. To offer more flexibitliy, we now added a text box to the 'SCP networks' menu that allows specification of the font sizes for node labels in addition to the text box for the specification of node diameters that we added the last time.
