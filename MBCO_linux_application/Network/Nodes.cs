...
namespace Network
{
    ...
    class NetworkNode_line_class
    {
        ...
        public void Set_name_as_name_plus_first_setNo_if_only_one_set()
        {
            int setNos_length = this.Sets_that_contain_nodes.Length;
            if (setNos_length > 1) { throw new Exception(); }
            else if (setNos_length == 1)
            {
                Name = Name + " - " + Sets_that_contain_nodes[0];
            }
        }
    ...
    class NetworkNode_class
    {
        ...
        public void Check_if_nodes_does_not_contain_input_node(string node)
        {
            foreach (NetworkNode_line_class node_line in this.Nodes)
            {
                if (node_line.Name.Equals(node))
                {
                    throw new Exception();
                }
            }
        ...
        public int Get_level_of_scps_and_check_if_all_scps_are_from_same_level(string[] scps)
        {
            ...
            int stringCompare = 0;
            int level_of_firstScp = 0;
            bool scp_part_of_nodes = false;

            for (int indexScp = 0; indexScp < scps_length; indexScp++)
            {
                scp = scps[indexScp];
                scp_part_of_nodes = false;
                stringCompare = -2;
                while ((indexN < nodes_length) && (stringCompare < 0))
                {
                    node_line = this.Nodes[indexN];
                    stringCompare = node_line.Name.CompareTo(scp);
                    if (stringCompare < 0)
                    {
                        indexN++;
                    }
                    ...
                    {
                        scp_part_of_nodes = true;
                        if (indexScp == 0)
                        {
                            level_of_firstScp = node_line.Level;
                        }
                        if (level_of_firstScp != node_line.Level)
                        {
                            throw new Exception();
                        }
                    }
                }
                if (!scp_part_of_nodes) { throw new Exception(); }
            }
            return level_of_firstScp;
        }
        #endregion
        ...
        public string[] Get_all_ordered_nodeNames_of_indicated_namespace(Namespace_type_enum namespace_type)
        {
            ...
            return nodeNames.OrderBy(l=>l).ToArray();
        }
        ...
        public void Set_level_for_selected_nodes(string[] selected_nodes, int level)
        {
            selected_nodes = selected_nodes.Distinct().OrderBy(l => l).ToArray();
            string selected_node;
            int selected_nodes_length = selected_nodes.Length;
            int indexSelected = 0;
            int stringCompare = -2;
            int nodes_length = this.Nodes_length;
            NetworkNode_line_class node_line;
            for (int indexNode = 0; indexNode < nodes_length; indexNode++)
            {
                node_line = this.Nodes[indexNode];
                stringCompare = -2;
                while ((indexSelected < selected_nodes_length) && (stringCompare < 0))
                {
                    selected_node = selected_nodes[indexSelected];
                    stringCompare = selected_node.CompareTo(node_line.Name);
                    if (stringCompare < 0)
                    {
                        indexSelected++;
                    }
        ...
        public void Set_name_as_name_plus_first_setNo_if_only_one_set()
        {
            foreach (NetworkNode_line_class node_line in this.Nodes)
            {
                node_line.Set_name_as_name_plus_first_setNo_if_only_one_set();
            }
        }
        ...
        public string Get_name_as_name_plus_setNos_of_nodeName(string nodeName)
        {
            int nodes_length = this.Nodes.Length;
            NetworkNode_line_class node_line;
            for (int indexN = 0; indexN < nodes_length; indexN++)
            {
                node_line = this.Nodes[indexN];
                if (node_line.Name.Equals(nodeName))
                {
                    return Get_name_as_name_plus_setNos(node_line);
                }
            }
            return "nodeName does not exist";
        }

        public void Set_name_as_name_plus_setNos()
        {
            StringBuilder sb = new StringBuilder();
            foreach (NetworkNode_line_class node_line in this.Nodes)
            {
                node_line.Name = Get_name_as_name_plus_setNos(node_line);
            }
        }
        #endregion

        public void Add_set_to_nodes(NetworkNodes_set_line_class add_set, params string[] nodeNames_of_indicated_set)
        {
            nodeNames_of_indicated_set = nodeNames_of_indicated_set.Distinct().OrderBy(l => l).ToArray();
            int nodeNames_length = nodeNames_of_indicated_set.Length;
            string nodeName;
            this.Order_by_name();
            NetworkNode_line_class node_line;
            int indexNode = 0;
            int nodes_length = this.Nodes.Length;
            int stringCompare = -2;
            List<NetworkNodes_set_line_class> new_sets = new List<NetworkNodes_set_line_class>();
            for (int indexNodeName = 0; indexNodeName < nodeNames_length; indexNodeName++)
            {
                nodeName = nodeNames_of_indicated_set[indexNodeName];
                stringCompare = -2;
                while ((indexNode < nodes_length) && (stringCompare < 0))
                {
                    node_line = this.Nodes[indexNode];
                    stringCompare = node_line.Name.CompareTo(nodeName);
                    if (stringCompare < 0)
                    {
                        indexNode++;
                    }
                    ...
                    {
                        new_sets.Clear();
                        new_sets.AddRange(node_line.Sets_that_contain_nodes);
                        new_sets.Add(add_set);
                        node_line.Sets_that_contain_nodes = new_sets.ToArray();
                    }
                }
                if (stringCompare != 0) { throw new Exception("Node name does not exist"); }
            }
        }
        ...
        private void Set_nodeColor_gradient(ref yed_node_color_line_class yED_node_color_line, float nodeValue, float maxValue, float minValue)
        {
            if (nodeValue <= 0)
            {
                //yED_node_color_line.Hexadecimal_colors = new string[] { Math_class.Get_hexadecimal_code(150, 150, 150) };
            }
            else
            {
                // yED_node_color_line.Hexadecimal_colors = new string[] { Get_hexadecimal_nodeColor_gradient(nodeValue, maxValue, minValue) };
            }
        }
        ...
        private yed_node_color_line_class Generate_legend_node(float value, float max_value, float min_value)
        {
            ...
            // yed_node_color_line.Hexadecimal_colors = new string[] { Get_hexadecimal_nodeColor_gradient(value, max_value, min_value) };
        ...
        public yed_node_color_line_class[] Get_yED_node_colors_based_on_level()
        {
            ...
            for (int indexN = 0; indexN < nodes_length; indexN++)
            {
                ...
                color_specification_line.Fill_color = Mbc_level_color_class.Get_node_color_for_indicated_level(node_line.Level);
        ...
        public yed_node_color_line_class[] Get_yED_node_colors_based_on_sets_if_not_indicated_different_in_dictionary(Dictionary<string, List<Color>> nodeLabel_colors_dict)
        {
            NetworkNode_line_class node_line;
            int nodes_length = this.Nodes_length;
            ...
            for (int indexN = 0; indexN < nodes_length; indexN++)
            {
                ...
                current_colors_length = nodeLabel_colors_dict[new_yED_node_color_line.NodeName].Count;
